...
test.describe("E2E Test Suite", () => {
  ...
  test("Add status fieled in table", async () => {
    ...
    await functions.fill_Text(
      pageobject.optioninput,
      "Member, Prospect, Lapsed"
    );
    // click on next button
    await functions.submit();
  ...
  test("Add status field in list view", async () => {
    ...
    await customAssert("Add status field in view", async () => {
      ...
      await functions.drag_And_Drop(
        pageobject.fieldsourrce,
        pageobject.newcolumn6
      );
      await page.selectOption(pageobject.fielddropdown, { label: "Status" });
      await page.selectOption(pageobject.fieldViewdropdown, {
        label: "as_text",
      });
    });
    ...
    await customAssert(
      "Assert the visibility of status field on list view",
      async () => {
  ...
  test("Create new page for Filter", async () => {
    ...
    await customAssert(
      "Select NewView_List in view to show dropdown",
      async () => {
    ...
    await customAssert("Select Member in status dropdown", async () => {
      await page.selectOption(pageobject.pagestatusdropdown, {
        label: "Member",
      });
    });
  ...
  test("Add Toggle on Filter View", async () => {
    ...
    await customAssert(
      "Assert the prospects toggle button on page",
      async () => {
  ...
  test("Add checkbox and search box in filter view", async () => {
    ...
    await customAssert(
      "Select checkbox_group in field view dropdown",
      async () => {
        await page.selectOption(pageobject.fieldViewdropdown, {
    ...
    await customAssert(
      "Member checkbox in searchbar dropdown should be visible",
      async () => {
