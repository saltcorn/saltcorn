...
test.describe("E2E Test Suite", () => {
  ...
  test("Check link settings", async () => {
    ...
    await functions.fill_Text(
      pageobject.linkurllocator,
      "https://www.youtube.com/@saltcorn"
    );
  ...
  test("Check search settings", async () => {
    ...
    const hasDropdownCheckbox = await page.locator(
      pageobject.hasdropdowncheckbox
    );
    await expect(hasDropdownCheckbox).not.toBeChecked();

    // validate that check box should not be checked
    const showStateBadgesCheckbox = await page.locator(
      pageobject.statebadgecheckbox
    );
  ...
  test("Create a new page with random string and navigate to new site URL", async ({
    browser,
  }) => {
    // Generate a random string
    const randomString = PageFunctions.generate_Random_String(10);
    ...
    expect(page.url()).toBe(
      baseURL + derivedURL + "page/" + "My_project_" + randomString
    );
