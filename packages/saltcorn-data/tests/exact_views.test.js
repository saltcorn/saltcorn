...
describe("Show view", () => {
  it("should render exactly", async () => {
    ...
    await test_show({
      ...
      response: `<div class="row"><div class="col-6"><div class="card shadow mt-4 " ><div class="card-body"><button class="btn btn-secondary btn-sm" onClick="ajax_modal('/view/authorshow?id=1')">foo it</button></div></div></div><div class="col-6"><div class="text-left" style="min-height: 100px;border: 1px solid black;  background-color: #a9a7a7; "><a href="https://countto.com/967"  >Herman Melville</a></div></div></div>`,
    });
    await test_show({
      layout: {
        type: "card",
...
describe("Filter view", () => {
  it("should render exactly", async () => {
    await test_filter({
      ...
      viewname: "testfilter",
      response:
        '<div class="row"><div class="col-6"><select name="ddfilterpatients.favbook.name" class="form-control d-inline" style="width: unset;" onchange="this.value==\'\' ? unset_state_field(\'patients.favbook.name\'): set_state_field(\'patients.favbook.name\', this.value)"><option value=""  /><option value="Kirk Douglas" >Kirk Douglas</option><option value="Michael Douglas" >Michael Douglas</option></select></div><div class="col-6"><button class="btn btn-outline-primary" onClick="set_state_field(\'pages\', encodeURIComponent(\'13\'))">thirteen</button></div></div><button class="btn btn-outline-primary" onClick="set_state_field(\'patients.favbook.name\', encodeURIComponent(\'Jim\'))">Jim</button><select name="ddfilterauthor" class="form-control d-inline" style="width: unset;" onchange="this.value==\'\' ? unset_state_field(\'author\'): set_state_field(\'author\', this.value)"><option value=""  /><option value="Herman Melville" >Herman Melville</option><option value="Leo Tolstoy" >Leo Tolstoy</option></select>',
    });
  });
});
describe("Page", () => {
