 ...
 *    while another suite's File.find() is listing the folder, so readdir()
 *    lists a name that stat() then can't find - File.find throws "File not
 *    found". A per-process store removes the cross-suite race. The run-tests
 *    CLI cleans up these directories after (and before) each run.
 *
