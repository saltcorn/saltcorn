...
class InfoCommand extends Command {
  ...
  async run() {
    ...
    const res = {
      ...
      cliPath,
      databaseVendor: db.sql_backend_display_name,
    ...
    const envVars =
      "DATABASE_URL SQLITE_FILEPATH PGDATABASE PGUSER PGHOST PGPORT PGPASSWORD PGDATABASE SALTCORN_SESSION_SECRET SALTCORN_MULTI_TENANT SALTCORN_MULTI_NODE SALTCORN_FILE_STORE SALTCORN_DEFAULT_SCHEMA SALTCORN_FIXED_CONFIGURATION SALTCORN_EXPOSED_CONFIGURATION SALTCORN_FIXED_PLUGIN_CONFIGURATION SALTCORN_INHERIT_CONFIGURATION SALTCORN_SERVE_ADDITIONAL_DIR SALTCORN_NWORKERS SALTCORN_DISABLE_UPGRADE PUPPETEER_CHROMIUM_BIN HTTPS_PROXY".split(
        " ",
      );
    envVars.forEach((v) => {
