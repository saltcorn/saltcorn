...
running command...
$ saltcorn (--version)
@saltcorn/cli/1.7.0-beta.1 linux-x64 node-v24.15.0
...
* [`saltcorn dev:release-resume VERSION [TAG]`](#saltcorn-devrelease-resume-version-tag)
...
_See code: [src/commands/add-schema.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/add-schema.js)_
...
_See code: [src/commands/backup.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/backup.js)_
...
```
USAGE
  $ saltcorn build-app [--allowShareTo] [--tenantAppName <value>] [-p <value>...] [-e <value>] [-t <value>]
    ...
    [--autoPublicLogin] [--showContinueAsPublicUser] [--allowOfflineMode] [--syncOnReconnect] [--syncOnAppResume]
    [--pushSync] [--syncInterval <value>] [--pushSyncHeartbeatInterval <value>] [--noProvisioningProfile]
    ...
    [--googleServicesFile <value>] [--remoteSchemaUrl <value>] [--remoteApiKey <value>]
    [--remotePushNotificationsEnabled] [--remoteBundleOutput <value>]

FLAGS
  -b, --buildDirectory=<value>
      A directory where the app should be build
  ...
  -e, --entryPoint=<value>
      This is the first view or page (see -t) after the login.

  -p, --platforms=<value>...
      Platforms to build for, space separated list

  -s, --serverURL=<value>
      URL to a saltcorn server, must start with https://

  -t, --entryPointType=<value>
      Type of the entry point ('view' or 'page'). The default is 'view'.

  -u, --userEmail=<value>
      Email of the user building the app

  --allowOfflineMode
      Switch to offline mode when there is no internet, sync the data when a connection is available again.

  --allowShareTo
      Allow sharing from other apps to this app

  --androidKeyStoreAlias=<value>
      A unique name to identify the key within the keystore file.

  --androidKeystore=<value>
      A self-signed certificate that includes the private key used to sign your app.

  --androidKeystorePassword=<value>
      he password to access the keystore file.

  --appGroupId=<value>
      An app group identifier to share data between the main app and the share extension on iOS, e.g.
      group.com.saltcorn.myapp

  --appIcon=<value>
      A png that will be used as launcher icon. The default is a png of a saltcorn symbol.

  --appId=<value>
      Id of the mobile app (default com.saltcorn.mobileapp)

  --appName=<value>
      Name of the mobile app (default SaltcornMobileApp)

  --appVersion=<value>
      Version of the mobile app (default 0.0.1)

  --autoPublicLogin
      Show public entry points before the login as a public user.
  ...
  --includedPlugins=<value>...
      Names of plugins that should be bundled into the app.If empty, no modules are used.

  --noProvisioningProfile
      Do not use a provisioning profile, only for simulator builds (iOS only)

  --provisioningProfile=<value>
      This profile will be used to sign your app

  --pushSync
      When offline mode is enabled, synchronize the synchedTables tables when a push notification is received.

  --pushSyncHeartbeatInterval=<value>
      Interval (in minutes) at which the device re-registers its push sync subscription to keep it alive. 0 or unset
      disables the heartbeat.

  --remoteApiKey=<value>
      API key (bearer token) for an admin user on the server given in --remoteSchemaUrl.

  --remoteBundleOutput=<value>
      Only prepare buildDir/www (schema, plugin bundles, site assets - everything saltcorn-specific) and write it as a zip
      file at this path, and exit - never touches Capacitor/native platforms. Used by the /api/mobile-app/build-bundle
      server route; not intended for direct use.

  --remotePushNotificationsEnabled
      Whether the server given in --remoteSchemaUrl has APN push configured - used instead of this machine's own local
      config to decide whether to bundle push support.

  --remoteSchemaUrl=<value>
      Base URL of a saltcorn server (e.g. https://example.com) to fetch the schema/config snapshot from -
      /api/mobile-app/build-bundle is appended automatically - instead of reading the local database. Use this when
      building on a machine other than the one the app will connect to at runtime (e.g. a Mac used only for the native iOS
      build), so table/view ids always match the live server instead of a separately-restored copy.

  --shareExtensionProvisioningProfile=<value>
  ...
  --syncInterval=<value>
      Perdiodic interval (in minutes) to run synchronizations in the background. This is just a min interval, depending on
      system conditions, the actual time may be longer.

  --syncOnAppResume
      When offline mode is enabled, synchronize the synchedTables tables when the app is resumed.

  --syncOnReconnect
      Run Synchronizations and return into online mode when the network connection is restored. When disabled, you still
...
_See code: [src/commands/build-app.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/build-app.js)_
...
_See code: [src/commands/build-capacitor-builder.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/build-capacitor-builder.js)_
...
_See code: [src/commands/configuration-check.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/configuration-check.js)_
...
_See code: [src/commands/configuration-check-backups.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/configuration-check-backups.js)_
...
_See code: [src/commands/create-tenant.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/create-tenant.js)_
...
_See code: [src/commands/create-user.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/create-user.js)_
...
_See code: [src/commands/delete-tenants.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/delete-tenants.js)_
...
_See code: [src/commands/delete-user.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/delete-user.js)_
...
_See code: [src/commands/dev/build.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/build.js)_
...
_See code: [src/commands/dev/localize-plugin.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/localize-plugin.js)_
...
_See code: [src/commands/dev/make-migration.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/make-migration.js)_
...
_See code: [src/commands/dev/plugin-test.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/plugin-test.js)_
...
_See code: [src/commands/dev/post-release.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/post-release.js)_
...
_See code: [src/commands/dev/release.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/release.js)_

## `saltcorn dev:release-resume VERSION [TAG]`

Resume a failed release at the cli publish step

```
USAGE
  $ saltcorn dev:release-resume VERSION [TAG] [-t <value>]

ARGUMENTS
  VERSION  New version number
  TAG      NPM tag to give this release (default: next)

FLAGS
  -t, --tag=<value>  NPM tag (alternative to the tag argument)

DESCRIPTION
  Resume a failed release at the cli publish step
```

_See code: [src/commands/dev/release-resume.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/release-resume.js)_
...
_See code: [src/commands/dev/serve.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/serve.js)_
...
_See code: [src/commands/dev/translate.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/dev/translate.js)_
...
_See code: [src/commands/fixtures.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/fixtures.js)_
...
_See code: [src/commands/get-cfg.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/get-cfg.js)_
...
_See code: [src/commands/info.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/info.js)_
...
_See code: [src/commands/inspect.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/inspect.js)_
...
_See code: [src/commands/install-pack.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/install-pack.js)_
...
_See code: [src/commands/install-plugin.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/install-plugin.js)_
...
_See code: [src/commands/list-tenants.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/list-tenants.js)_
...
_See code: [src/commands/list-triggers.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/list-triggers.js)_
...
_See code: [src/commands/list-users.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/list-users.js)_
...
_See code: [src/commands/migrate.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/migrate.js)_
...
_See code: [src/commands/modify-user.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/modify-user.js)_
...
_See code: [src/commands/plugins.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/plugins.js)_
...
_See code: [src/commands/pre-install-modules.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/pre-install-modules.js)_
...
_See code: [src/commands/prepare.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/prepare.js)_
...
_See code: [src/commands/reset-schema.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/reset-schema.js)_
...
_See code: [src/commands/restore.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/restore.js)_
...
_See code: [src/commands/rm-tenant.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/rm-tenant.js)_
...
_See code: [src/commands/run-benchmark.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/run-benchmark.js)_
...
_See code: [src/commands/run-js.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/run-js.js)_
...
_See code: [src/commands/run-sql.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/run-sql.js)_
...
```
USAGE
  $ saltcorn run-tests [PACKAGE] [-c] [-l] [-v] [-d] [-t <value>] [-f <value>] [--watch] [--watchAll]
    [--database <value>]
...
FLAGS
  ...
  -f, --file=<value>        Run only the given test file (jest: path pattern; node:test packages: path relative to the
                            package's dist directory, e.g. tests/table.test.js)
...
_See code: [src/commands/run-tests.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/run-tests.js)_
...
_See code: [src/commands/run-trigger.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/run-trigger.js)_
...
_See code: [src/commands/scheduler.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/scheduler.js)_
...
_See code: [src/commands/serve.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/serve.js)_
...
_See code: [src/commands/set-cfg.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/set-cfg.js)_
...
_See code: [src/commands/set-daily-time.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/set-daily-time.js)_
...
_See code: [src/commands/setup.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/setup.js)_
...
_See code: [src/commands/setup-benchmark.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/setup-benchmark.js)_
...
_See code: [src/commands/sync-upload-data.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/sync-upload-data.js)_
...
_See code: [src/commands/take-snapshot.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/take-snapshot.js)_
...
_See code: [src/commands/transform-field.js](https://github.com/saltcorn/saltcorn/blob/v1.7.0-beta.1/packages/saltcorn-cli/src/commands/transform-field.js)_
<!-- commandsstop -->
