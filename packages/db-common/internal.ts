 ...
 * @returns {string}
 */
export const sqlsanitize = (nm: string | any): string => {
 ...
 * @returns {string}
 */
export const sqlsanitizeAllowDots = (nm: string | any): string => {
  if (typeof nm === "symbol") return sqlsanitizeAllowDots(nm.description);
  const s = nm.replace(/[^A-Za-z_0-9."]*/g, "");
  if (s[0] >= "0" && s[0] <= "9") return `_${s}`;
  else return s;
};
 ...
 * @returns {string}
 */
const whereFTS = (v: any, i: number, is_sqlite: boolean): string => {
 ...
 * @returns {string}
 */
const placeHolder = (is_sqlite: boolean, i: number): string => (is_sqlite ? `?` : `$${i}`);
 ...
 * @returns {function}
 */
const subSelectWhere = (is_sqlite: boolean, i: number) => (k: any, v: any): string => {
  ...
  const where =
    whereObj && wheres.length > 0
      ...
      : "";
  return `${quote(sqlsanitizeAllowDots(k))} in (select ${v.inSelect.field
 ...
 * @returns {string}
 */
const quote = (s: string): string => (s.includes(".") || s.includes('"') ? s : `"${s}"`);
/**
 * @param {boolean} is_sqlite
 * @param {string} i
 * @returns {function}
 ...
 * @returns {function}
 */
const whereClause = (is_sqlite: boolean, i: any): ([k, v]: [string, any | [any, any] ]) => string =>
 ...
 * @returns {object}
 */
export const mkWhere = (whereObj: any, is_sqlite: boolean, initCount = 0): WhereAndVals => {
  const wheres = whereObj ? Object.entries(whereObj) : [];
  //console.log({ wheres });
  const where =
    whereObj && wheres.length > 0
...
type CoordOpts = {
  latField: string, longField: string, 
